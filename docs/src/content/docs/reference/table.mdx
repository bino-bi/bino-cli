...
The smallest well-formed `Table` sets the required `spec.dataset` plus the fields
...
`dataset` accepts the dataset name shown above, an inline [`DataSet`](/reference/dataset/)
...
repeated here (see [Inherited props](#inherited-props)).

## Fields

- `dataset` – required dataset reference: a name, an inline [`DataSet`](/reference/dataset/) definition, or an array of those.
- `filter` – optional filter expression.
- `order` – sort key; using scenario names requires them to appear in `scenarios`. Default: `category`. `auto` uses the key suggested by the dataset instead: subCategory if the dataset has subCategory values, else category, else rowGroup. With subCategory values the category rows keep their dataset order and `orderDirection` has no effect on them; `thereof` drilldown rows are sorted by subCategory, in the opposite direction to `orderDirection`. With only category values, `auto` and an omitted `order` sort the same.
...
- `type` – table type (`list`, `sum`, `opt`, `sumnototal`, `optnototal`). Only `sum` and `opt` render a grand-total row at the bottom of the table.
- `sumTitle` – label of that grand-total row. It is **not** a title above the table: it is the text in the category cell of the total row. Since the other table types render no total row, they ignore the value. When empty, the row is labelled with the IBCS grand-total symbol ❖.
...
- `unitScaling` – data units displayed per 1 em of bar length for absolute variance bars (d_ prefix columns) — map-scale semantics: a larger value means smaller bars. Also accepts `auto` (fit the largest bar to the bar column) or a [ScalingGroup](/reference/scaling-group/) name. Default: 100.
- `percentageScaling` – percentage points displayed per 1 em of pin length for relative variance pins (dr_ prefix columns); `2` means a +2 % pin is 1 em long. Also accepts `auto` or a scaling group name. Default: 2.
- `scale` – proportional scaling of the entire table (font size, em-based spacing, SVG bar charts, pins, patterns). All visual dimensions scale together.
  - *not set* – auto-scale to fit the parent width; emits an `AUTO_scale` warning with the applied factor. (An empty string `""` is rejected by the schema.)
...
`scale` only fits the width. A table whose content is taller than the space it
gets is clipped at the bottom by the page cell — the last rows, grand total
included, simply are not drawn. From template engine **v1.0.0-next.26** on the
engine reports this as a `WARN_overflow` diagnostic; a build with
[`--layout-state`](/cli/layout-state/) prints it as a `layout-overflow` finding,
and the preview inspector shows it on the component.

What helps: give the table more space (fewer components on the page, a taller
layout), show fewer rows (`limit`), or set a smaller fixed `scale`. Setting
`scale: auto` does not help — it fits the width, not the height.
...
|-----------|------|----------|---------|-------------|--------|
| `spec.dataset` | string, object, or array | yes | — | Dataset reference: a name, an inline [`DataSet`](/reference/dataset/) definition, or an array of those (at least one). Provided by the **withDataset** mixin in the template engine. | `dataset: revenue_by_customer` |
| `spec.filter` | string | no | `""` | AlaSQL WHERE clause fragment to filter rows before rendering. | `filter: "region = 'DE'"` |
| `spec.order` | string | no | `category` | Sort key. A scenario key must also appear in `scenarios`. Values: `auto`, `category`, `categoryindex`, `rowgroup`, `rowgroupindex`, `ac1`–`ac4`, `fc1`–`fc4`, `pp1`–`pp4`, `pl1`–`pl4`, `inherited-closest`, `inherited-page`. `auto` uses the dataset's suggested key instead of `category` (see [Fields](#fields)). | `order: ac1` |
...
| `spec.unitScaling` | string or number | no | `100` | Name of a scaling group, `auto`, or data units per em of bar length for absolute variance bars (`d_` prefix columns). Larger value = smaller bars. | `unitScaling: 50` |
| `spec.percentageScaling` | string or number | no | `2` | Name of a scaling group, `auto`, or percentage points per em of pin length for relative variance pins (`dr_` prefix columns). | `percentageScaling: 1` |
...
| `spec.thereof` | string or array | no | `[]` | Drilldown rows (subCategory level), as a YAML array or a JSON string. See [Thereof](#thereof-drilldown-rows). | see below |
...
| `spec.partof` | string or array | no | `[]` | Part-of groupings (category level), as a YAML array or a JSON string. See [Partof](#partof-category-groupings). | see below |
...
| `spec.columnthereof` | string, array, or null | no | `null` | Column-level drilldown configuration, as a YAML array or a JSON string. Mutually exclusive with `interval` (`columnthereof` wins). See [Columnthereof](#columnthereof-column-drilldown). | see below |
...
| `spec.attributes` | string or array | no | — | Computed attribute columns (header label → aggregate expression), in list order; also accepts a JSON object string whose key order defines the column order. See [Computed attribute columns](#computed-attribute-columns). | see below |
