...
The `bn-text` component in the template engine uses the following mixins:
- **withDataset** — subscribes to one or more datasets via the `datasets` HTML attribute (comma-separated). Maps from `spec.dataset` in the manifest.
- **withTranslation** — provides the `t(key)` function for i18n lookups via the `namespace` HTML attribute.
