...
- `t('<key>')` – looks the key up in the component's i18n namespace (set via `i18nNamespace`, default `_system`) for the artefact's language; an unknown key renders as the key itself. See the [Internationalization reference](/reference/internationalization/).
