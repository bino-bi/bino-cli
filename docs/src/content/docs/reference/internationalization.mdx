...
1. **The locale code must match exactly.** Components look up
   ...
   with `code: de-DE` passes schema validation but is **never** consulted when
   the report renders with `language: de` — only the `i18n-code-unused` lint
   points it out. Always use plain `de` or `en`.
...
This is the smallest well-formed `Internationalization`: a language code and the
...
[scoped override](#scoped-overrides-with-i18nnamespace).
...
A file scaffolded this way by bino v0.97.0 or earlier still sets the old `no-data`
labels, wrapped in `==` markers; update or delete those keys. The
`i18n-no-data-markers` lint reports them.
...
manifest still validates and the override is simply ignored, leaving that
engine's old built-in placeholder in place.
:::
...
- `i18n-no-data-markers` — warns when a `*.no-data` label is still `==No Data==` or `==Keine Daten==`.
