...
The `bn-internationalization` component in the template engine uses the following mixins:
- **withBodyObserver** — observes changes to the component's text content (the translation payload).
...
The `bn-internationalization` component accepts two props directly: `code` (language code, from `spec.code`) and `namespace` (from `spec.namespace`). Components that consume translations use the **withTranslation** mixin, which provides a `t(key, data?)` function. Keys starting with `.` are expanded to `<component-tag>.key`. If a key is not found in the component's namespace, the `_system` namespace is used as a fallback. Mustache-style interpolation (`{{variable}}`) is supported in translation values.
:::
