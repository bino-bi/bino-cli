...
- `from` — the slot the earlier row is read from, any of the sixteen slots.
- `shift` — how far back: `<n> <unit>` with the unit `day`, `week`, `month`, `quarter` or `year` (pattern `^[1-9][0-9]* (day|week|month|quarter|year)$`).
- `grain` — the period one row stands for, one of the same five units. It is **always required** and never inferred from the data.

The declarations work with `query`, `prql` and `source` alike. A slot may appear in `derive` or in `assert`, not both ([`dataset-derive-conflict`](/appendix/lint-rules/#dataset-derive-conflict)), and a derived slot must not be returned by the query.

**Identity and periods.** Rows are matched by identity — every non-measure column except `date`, so all dimension columns and their index twins — and by period, the `date` truncated to `grain` on both sides. Keying on the truncated period is what makes month ends line up: `2020-03-31` shifted by one month finds `2020-02-29`. The shift has to round-trip, so under grain `day` a one-month shift leaves the 31st empty rather than matching a clamped day. A row without a prior period gets `NULL`; the first month of a table has no `pp1`, its first year no `pp2`.
