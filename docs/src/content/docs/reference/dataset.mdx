...
- `derive` adds a column, never rows. An identity that is new in this period keeps its row with an empty slot. An identity that existed only in the prior period has no current row to attach the value to, so it does not appear at all. To show it as `ac1 = NULL, pp1 = <prior value>`, the query has to produce the row, for example by joining the actuals onto every identity × period:
  ...
  With `derive: { pp1: { from: ac1, shift: 1 month, grain: month } }` the faded identity's row then carries its `pp1`, and a new identity's first-period row is empty on both.
