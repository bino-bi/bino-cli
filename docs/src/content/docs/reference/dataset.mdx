...
The `bn-dataset` component in the template engine uses the following mixins:
- **withDatasource** — subscribes to raw datasources via the `datasources` attribute (comma-separated list).
- **withBodyObserver** — observes changes to the component's text content (the SQL/data payload).
...
The `bn-dataset` component accepts these props directly: `name` (from `metadata.name`), `datasources` (comma-separated datasource list), `static` (bypass AlaSQL, defaults to `false`), and `raw` (controlled internally by the CLI, defaults to `true`).
:::
