...
spec:
  ...
  selectedStyle: corporate-style   # optional ComponentStyle name
  ruleset: corporate-rules         # optional RuleSet name
  children:
    - kind: ChartStructure
      spec: { ... }
    - kind: Table
...
| `spec.titleNamespace` | string | no | `"_system"` | Deprecated: use `i18nNamespace`. Namespace for title translations only; never applies to children. | `titleNamespace: _system` |
...
| `spec.messageImage` | string | no | `""` | Name of an [Asset](/reference/asset/) document, or an absolute URL, for the message row image. | `messageImage: "logo"` |
