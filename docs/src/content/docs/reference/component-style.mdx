...
|-----------|------|----------|---------|-------------|--------|
| `spec.content` | object or string | yes | — | Style definitions as a YAML object or JSON string. Contains component-specific style sections. | see sections above |

#### Content structure

| Section | Description |
|---------|-------------|
| `content.global` | Global typography and color settings (inherited by all components). |
| `content.bn-text` | Style settings for text components. |
| `content.bn-chart-time` | Style settings for time/column charts. |
| `content.bn-chart-structure` | Style settings for structure/bar charts. |
| `content.bn-tree` | Style settings for hierarchical tree diagrams. |
| `content.bn-grid` | Style settings for grid layouts. |
...
The `bn-component-style` component in the template engine uses the following mixins:
- **withBodyObserver** — observes changes to the component's text content (the style payload).
...
The `bn-component-style` component accepts the `name` prop directly (mapped from `metadata.name`, defaults to `"_default"`). Components that consume styles use the **withComponentStyle** mixin, which provides a three-layer merge strategy: `_system` → `_default` → `selected-style`. Components can query styles via `componentStyle(s => s.table.headerBg)`.
:::
