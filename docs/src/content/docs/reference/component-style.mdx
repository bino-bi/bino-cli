...
This is the smallest well-formed `ComponentStyle`: the required `spec.content`
...
`spec.content` can also be written as a JSON string instead of a YAML object,
which is useful when copying from front-end configuration — see
[JSON string style](#json-string-style).
...
The style object accepts additional keys beyond these sections, so unknown or
misspelled style keys pass validation without a diagnostic.
...
This section is not described by the manifest schema. It is accepted because the
style object allows extra keys, so its properties and defaults are not validated
and a typo in a key stays silent.
...
Each scenario key contains an SVG style object. Further scenario keys are allowed
and take the same SVG style object.
...
| `spec.content.global` | object | no | — | Global typography and color settings, inherited by all components. See [Global style](#global-style). | `global: { fontSizePx: 14 }` |
| `spec.content.bn-text` | object | no | — | Style settings for text components. Reserved for future use, accepts any properties. See [Text style](#text-style-bn-text). | `bn-text: {}` |
| `spec.content.bn-chart-time` | object | no | — | Style settings for time/column charts: `scaleFactors`, `axisStyles`, `barStyles`, `varianceStyles`, `stackColors`, `padding`. See [Time chart style](#time-chart-style-bn-chart-time). | `bn-chart-time: { scaleFactors: { columnWidth: 2.5 } }` |
| `spec.content.bn-chart-structure` | object | no | — | Style settings for structure/bar charts: `scaleFactors`, `axisStyles`, `barStyles`, `varianceStyles`, `stackColors`, `padding`. See [Structure chart style](#structure-chart-style-bn-chart-structure). | `bn-chart-structure: { scaleFactors: { barWidth: 1.5 } }` |
| `spec.content.bn-tree` | object | no | — | Style settings for hierarchical tree diagrams. See [Tree chart style](#tree-chart-style-bn-tree). | `bn-tree: { edgeColor: "#606060" }` |
| `spec.content.bn-grid` | object | no | — | Style settings for grid layouts. Not described by the schema, so it is neither validated nor completed. See [Grid style](#grid-style-bn-grid). | `bn-grid: { cellPadding: 8 }` |

Components reference a named style through their own `spec.selectedStyle` attribute; see [Using named styles](#using-named-styles).
...
| Attribute | Type | Required | Default | Description | Sample |
|-----------|------|----------|---------|-------------|--------|
| `scaleFactors` | object | no | per property | Scale factors for element sizing, multiplied by `global.fontSizePx`. Defaults differ per chart: see [time chart](#scale-factors-time-chart) and [structure chart](#scale-factors-structure-chart). | `scaleFactors: { columnWidth: 2.5 }` |
| `axisStyles` | object | no | — | SVG styles for axis elements, keyed by scenario. See [SVG style group](#svg-style-group). | `axisStyles: { ac: { stroke: "#000000" } }` |
| `barStyles` | object | no | — | SVG styles for bar/column elements, keyed by scenario. See [SVG style group](#svg-style-group). | `barStyles: { ac: { fill: "#333333" } }` |
| `varianceStyles` | object | no | — | SVG styles for variance indicators, keyed by direction. See [SVG variance style group](#svg-variance-style-group). | `varianceStyles: { neg: { fill: "#ff0000" } }` |
...
| `padding` | object | no | `10` per side | Padding around the chart area. See [Padding](#padding). | `padding: { top: 15 }` |
