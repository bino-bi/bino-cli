...
spec:
  ...
  level: category              # rowgroup | category | subcategory (+ index variants) | auto
...
### Example: inherit from the closest card
...
| `spec.level` | string | no | auto-detect with warning | Aggregation level. Values: `rowgroup`, `rowgroupindex`, `category`, `categoryindex`, `subcategory`, `subcategoryindex`, `auto`. When `auto`, detected silently. When blank, detected with a warning. | `level: category` |
...
| `spec.scenarios` | string or array | no | — | Scenario measures (up to 4). Values: `ac1`–`ac4`, `fc1`–`fc4`, `pp1`–`pp4`, `pl1`–`pl4`, `auto`, `inherited-closest`, `inherited-page`. | `scenarios: ["ac1", "py1"]` |
