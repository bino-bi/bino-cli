...
- `unitScaling` – data units displayed per 1 em of bar width (map-scale semantics: larger value = smaller bars), `auto` to fit the available width, or the name of a [ScalingGroup](/reference/scaling-group/). Unset or invalid values auto-fit with a warning that reports the resolved value.
