...
The smallest well-formed `ChartStructure` sets the required `spec.dataset` plus the
...
[Stack configuration](#stack-configuration)), and `scenarios`, `variances`, `order`
...
- `order` / `orderDirection` – sort key and direction; if you use a scenario, it must appear in `scenarios`. When `order` is set to `auto`, it is detected from the dataset silently. When omitted, it is detected with a warning.
...
- `scenarios` – list of scenarios (up to 4); values are `ac1`–`ac4`, `fc1`–`fc4`, `pp1`–`pp4`, `pl1`–`pl4`. When omitted, they are detected from the dataset with a warning.
...
|-----------|------|----------|---------|-------------|--------|
| `spec.dataset` | string, object, or array | yes | — | Dataset reference: a name, an inline [`DataSet`](/reference/dataset/) definition, or an array of those (at least one). Provided by the **withDataset** mixin in the template engine. | `dataset: revenue_by_region` |
...
| `spec.order` | string | no | auto-detect with warning | Sort key. Values: `auto`, `category`, `categoryindex`, `rowgroup`, `rowgroupindex`, `ac1`–`ac4`, `fc1`–`fc4`, `pp1`–`pp4`, `pl1`–`pl4`, `inherited-closest`, `inherited-page`. When `auto`, detected silently. When omitted, detected with a warning. A scenario used here must also appear in `scenarios`. | `order: ac1` |
...
| `spec.stack` | object | no | — | Stacked bar rendering configuration. When set, bars are stacked instead of shown side-by-side. See [Stack configuration](#stack-configuration). | see below |
