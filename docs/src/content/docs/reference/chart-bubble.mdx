...
| `spec.scale` | string or number | no | auto-scale with warning | Proportional scaling. `"none"`: no scaling. `"auto"`: auto-scale silently. Positive number: fixed factor (ScalingGroup bubble areas follow the effective font size, so the factor applies squared to areas). | `scale: auto` |
