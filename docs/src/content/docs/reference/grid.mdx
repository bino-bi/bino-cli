...
|-----------|------|----------|---------|-------------|--------|
| `spec.chartTitle` | string | no | `""` | Title displayed at the top-left of the grid. | `chartTitle: "Sales Matrix"` |
| `spec.rowHeaders` | array | yes | — | Row header definitions. Simple strings (IDs auto-generated as 0, 1, 2…) or objects with `label` and `id`. | `rowHeaders: [North, South]` |
| `spec.columnHeaders` | array | yes | — | Column header definitions. Simple strings or objects with `label` and `id`. | `columnHeaders: [Q1, Q2, Q3]` |
...
| `spec.rowHeaderWidth` | string | no | `"auto"` | CSS width of the row header column. | `rowHeaderWidth: "120px"` |
| `spec.cellGap` | string | no | `"0px"` | CSS gap between grid cells. | `cellGap: "8px"` |
| `spec.children` | array | yes | — | Array of grid child definitions specifying position and content. | see below |
...
spec:
  children:
    - row: electronics        # must match rowHeaders ID
      ...
      kind: ChartStructure    # Text | Table | ChartStructure | ChartTime | Tree | Image
...
|----------|------|----------|---------|-------------|
| `children[].row` | string or integer | yes | — | Row identifier matching a row ID from `rowHeaders`. |
| `children[].column` | string or integer | yes | — | Column identifier matching a column ID from `columnHeaders`. |
| `children[].kind` | string | yes | — | Component type. Values: `Text`, `Table`, `ChartStructure`, `ChartTime`, `Tree`, `Image`. |
| `children[].ref` | string | no | — | Reference to standalone document by name. |
| `children[].optional` | boolean | no | `false` | Skip gracefully if `ref` is missing. |
| `children[].spec` | object | conditional | — | Component specification. Required for inline children, optional override for `ref` children. |
| `children[].metadata` | object | no | — | Optional metadata including `name` and `constraints`. |

:::note[Template Engine]
The `bn-grid` component in the template engine uses the following mixins:
...
- **withComponentStyle** — provides themed styling via three-layer merge (system → default → selected). Grid-specific styles are configured in the `bn-grid` section of ComponentStyle.
- **withHasError** — manages validation errors/warnings via the `has-error` attribute.

The `bn-grid` component accepts `rowHeaders` and `columnHeaders` as JSON strings. In the YAML manifest, prefer the YAML array form which the CLI serializes to JSON for the template engine.
:::
