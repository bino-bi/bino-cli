...
| `spec.order` | string | no | auto-detect with warning | Sort key; a scenario used here must also appear in `scenarios`. Values: `category`, `categoryindex`, `rowgroup`, `rowgroupindex`, `ac1`–`ac4`, `fc1`–`fc4`, `pp1`–`pp4`, `pl1`–`pl4`, `inherited-closest`, `inherited-page`. | `order: ac1` |
