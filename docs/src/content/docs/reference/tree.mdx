...
- `kind` (required) – type of component: `Label`, `Table`, `ChartStructure`, `ChartTime`, `ChartScatter`, `ChartBubble`, `Image`, or `LayoutCard`
...
| `nodes[].params` | object | no | — | Parameter values (string map) passed to the referenced document. Only valid together with `ref`. |
