...
The `bn-tree` component in the template engine uses the following mixins:
...
- **withComponentStyle** — provides themed styling via three-layer merge (system → default → selected). Tree-specific styles are configured in the `bn-tree` section of ComponentStyle.
- **withHasError** — manages validation errors/warnings via the `has-error` attribute.

The `bn-tree` component accepts the `edges` prop as a JSON string. In the YAML manifest, prefer the YAML array form which the CLI serializes to JSON for the template engine.
:::
