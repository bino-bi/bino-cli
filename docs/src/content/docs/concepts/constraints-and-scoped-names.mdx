...
| Kind         | Description                                    | Artefact Type        |
| ------------ | ---------------------------------------------- | -------------------- |
...
Documents installed from a registry keep their scoped package name
(e.g. `@acme/base-layout`) as `metadata.name`, so `@scope/name` is a valid
document name. Installed packages participate in the same per-artefact
uniqueness rules: a registry package that collides with a local document of
the same kind fails the duplicate-name validation. See the
[Registry and dependencies guide](/guides/registry-and-dependencies/).
