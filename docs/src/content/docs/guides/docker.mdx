...
<Aside type="note">
A read-only `/work` works when `--out-dir` points at a writable path, such as a second mount:
...
docker run --rm \
  -v "$PWD:/work:ro" \
  -v "$PWD/out:/out" \
  ghcr.io/bino-bi/bino-cli:latest build --out-dir /out
```

bino then builds without its dataset cache and keeps scratch files in `/tmp`, which must stay
writable. Pass `--out-dir` as an absolute path like `/out`: a relative one such as `dist/` resolves
inside `/work`.
</Aside>
...
`bino preview` binds `127.0.0.1` by default. Inside a container that is the *container's* own
...
Then open `http://localhost:8080/`. This works the same on Linux, macOS and Windows — no
`--network host`, which Docker Desktop does not support in a useful way anyway.

<Aside type="caution">
A wildcard bind exposes everything the preview server serves to anything that can reach the
published port, including `POST /__explorer/query`, which runs arbitrary DuckDB SQL against your
project with no authentication. Publish to loopback only (`-p 127.0.0.1:8080:8080`) unless you
control the network.
</Aside>
