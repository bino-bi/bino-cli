...
<img src={dashboardImg.src} alt="A parameterized dashboard page rendered with region-specific data" style="max-width:100%;border:1px solid var(--sl-color-gray-5);border-radius:0.5rem;margin:1rem 0;" />
...
Choice from predefined options. Each item has a `value` (the key used in code) and an optional `label` (the human-readable display name).
...
For select parameters, you can access both the value and the label:
- `${REGION}` – returns the value (e.g., `"EU"`)
- `${REGION_LABEL}` – returns the label (e.g., `"Europe"`)
...
Date and time value in ISO 8601 format (e.g. `2024-01-15T14:30:00Z`).
  ...
  children:
    ...
    - kind: Table
      spec:
        dataset: sales-data
        type: sum
        sumTitle: "${REGION_LABEL} Sales"

    - kind: ChartStructure
      spec:
        dataset: regional-data
        chartTitle: "${YEAR} Performance - ${REGION_LABEL}"
```

The `_LABEL` suffix is available for any `select` type parameter with defined options. If the label is not defined for an item, the value is used instead.

## Parameters on referenced components

Standalone component documents (Text, Table, ChartStructure, ChartTime, Tree, Grid, LayoutCard, Image) declare parameters in `metadata.params` exactly like a LayoutPage. Anything that references such a document with `ref` — a layout child, grid child, or tree node — passes values via `params`:

```yaml
apiVersion: bino.bi/v1alpha1
kind: Text
metadata:
  name: regional-commentary
  params:
    - name: REGION
      type: string
      default: EU
...
Precedence, defaults, required params, and `${NAME_LABEL}` work identically to LayoutPage params. This is especially useful for predef packages installed from a registry — see [Registry and dependencies](/guides/registry-and-dependencies/).
