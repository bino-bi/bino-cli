...
- A region that stops reporting disappears from the months it has no row for; `derive` never adds rows. To keep showing it with an empty actual and its previous period, scaffold the query over every region × month, see [How a row is matched](/reference/dataset/#how-a-row-is-matched).
