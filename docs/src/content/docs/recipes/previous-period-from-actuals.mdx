...
- A region that stops reporting keeps a row for the month after its last one, with an empty actual and its previous period filled, so the drop is visible; see [How a row is matched](/reference/dataset/#how-a-row-is-matched).
