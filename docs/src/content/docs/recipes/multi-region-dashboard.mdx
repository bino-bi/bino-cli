...
<img src={dashboardImg.src} alt="A multi-region operations dashboard with KPIs, trend chart, and data table" style="max-width:100%;border:1px solid var(--sl-color-gray-5);border-radius:0.5rem;margin:1rem 0;" />
...
Add an `Internationalization` manifest to translate labels per language. The `code`
must exactly equal the artefact's `language` (`de` or `en`); free-form keys like these
are referenced from `Text` components via `${t('deliveries_total')}`:
