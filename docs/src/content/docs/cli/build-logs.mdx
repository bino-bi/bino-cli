...
| `dist/bino-build-<id>.json` | JSON   | When `--build-log-format=json` or `--embed-data-csv` is used |
...
- `--build-log-format=json` — Explicitly request JSON output
...
| --------------------------- | ------ | -------- | --------------------------------------- |
| `--build-log-format`              | string | `"text"` | Build log format: `"text"` or `"json"`  |
...
```bash
bino build --build-log-format=json
```
...
```bash
bino build --build-log-format=json --embed-data-csv --detailed-execution-plan
```
...
```yaml
- name: Build Reports
  run: bino build --build-log-format=json --detailed-execution-plan
...
build:
  script:
    - bino build --build-log-format=json --detailed-execution-plan
  artifacts:
    paths:
      - dist/*.pdf
      - dist/*.json
