...
The template engine is cached in `~/.bino/cdn/bn-template-engine/` and multiple versions can coexist. By default, bino uses the latest installed version unless a specific version is pinned in `bino.toml`.
