...
Chrome headless shell (`chrome-headless-shell`) is a lightweight Chrome binary purpose-built for headless rendering. It is downloaded from Google's Chrome for Testing infrastructure and cached in `~/.bn/chrome-headless-shell/`.
...
The template engine (`bn-template-engine`) is a JavaScript runtime used to render HTML templates. It is downloaded separately from Chrome headless shell and cached in `~/.bn/cdn/bn-template-engine/`.
