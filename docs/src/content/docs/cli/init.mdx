...
canonical folders. `predef` scaffolds a [predef project](/concepts/project-configuration/#predef-packages):
an ordinary project that also authors a reusable package, with a `[package]` table in
`bino.toml`, namespaced manifests and a `mocks/` preview harness. A source may also be a
remote template — see [Project Templates](/concepts/templates/) for the full grammar,
...
- `--name` – internal report name; for `predef` the package name segment (built-in templates).
- `--title` – human-readable report title (built-in templates).
- `--language` – language code, for example `en`, `de` (built-in templates).
- `--set key=value` – set a template field value; repeatable. Remote/local templates declare
  their own fields; `predef` accepts the `[package]` fields `Scope`, `Description`,
  `Visibility`, `Tags` (comma-separated) and `Category`.
...
Scaffold a predef project that authors a reusable package:

```bash
bino init predef -d starter-kit
```

The `predef` wizard asks for the `[package]` table instead of the report fields:
registry scope, package name, description, visibility, tags and category. To skip
the wizard, pass them with `--set`:

```bash
bino init predef -d starter-kit --set Scope=myorg --set Visibility=public -y
```
...
build-log-format = "json"    # Build log file format: text or json
