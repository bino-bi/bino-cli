...
If `bino` is not on your `PATH`, you can configure a custom path later via the `bino.binPath` setting.
...
If something is wrong, the command shows a detailed message and may offer quick actions (for example, opening settings to configure `bino.binPath`).

### 3.2 Configure `bino.binPath` (optional)

If the CLI is not on your `PATH`, set a custom path in your VS Code settings:

```jsonc
// .vscode/settings.json
{
  "bino.binPath": "/absolute/path/to/bino"
}
```

Or via **Settings UI**: search for **"Bino: Bin Path"**.
...
| ----------------------- | ------- | ------- | ------------------------------------------------------------- |
| `bino.binPath`          | string  | ""      | Path to the `bino` CLI. If empty, uses `bino` from `PATH`.    |
...
```jsonc
{
  "bino.binPath": "/usr/local/bin/bino",
...
- Fix:
  - Ensure the CLI is installed and on your `PATH`, or
  - Set `bino.binPath` to the absolute path of the binary.
