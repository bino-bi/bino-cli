...
- Linux x86_64, arm64
- Windows x86_64
- Linux containers (`linux/amd64`, `linux/arm64`) via the official image
...
<Aside type="caution">
  PostgreSQL and MySQL database connections are not available on Windows. The required DuckDB extensions are currently not supported on that platform. CSV, Excel, and other file-based data sources work normally.
</Aside>
...
For a graphical installation experience, download the setup wizard:

[bino-cli_Windows_x86_64_setup.exe](https://github.com/bino-bi/bino-cli-releases/releases/latest/download/bino-cli_Windows_x86_64_setup.exe)

The installer will:
1. Show the license agreement
2. Let you choose the install directory (default: `%LOCALAPPDATA%\bino`)
3. Install `bino.exe` and `duckdb.dll`
4. Add the install directory to your user PATH
5. Run `bino setup` to download the browser runtime

To uninstall, use **Add or Remove Programs** in Windows Settings.

<Aside type="tip">
  The installer also supports silent mode for scripted deployments: `bino-cli_Windows_x86_64_setup.exe /SILENT`
</Aside>

## Install via Homebrew (macOS and Linux)

If you use [Homebrew](https://brew.sh/), you can install bino from our tap:
...
<Aside type="tip">
  Using Homebrew is convenient because it handles updates alongside your other packages.
</Aside>
...
## Run bino in a container

If you would rather not install anything, the official image already contains bino *and* its
rendering engine — there is no `bino setup` step:

```bash
docker run --rm -v "$PWD:/work" ghcr.io/bino-bi/bino-cli:latest build --out-dir dist/
```

Your project is mounted at `/work`, and the PDFs land in `dist/` on your machine.

See [Running bino in Docker](/guides/docker/) for bind mounts, file ownership, and offline behaviour,
and [Recipe: Deploy a live dashboard](/recipes/live-dashboard-deployment/) for serving a
`LiveReportArtefact`.
...
- Prefer containers? See [Running bino in Docker](/guides/docker/).
