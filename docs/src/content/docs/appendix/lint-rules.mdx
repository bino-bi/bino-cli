...
| `dataset-required`             | Dataset Required             | Warning  | Table, ChartStructure, ChartTime, ChartScatter, and ChartBubble components must have a `spec.dataset` binding. |
...
Data visualization components (`Table`, `ChartStructure`, `ChartTime`, `ChartScatter`, `ChartBubble`) need a dataset to display. Without `spec.dataset`, there's nothing to render.
