...
type InitBundleInput struct {
	...
	Source    string            `json:"source,omitempty" jsonschema:"template source; empty selects the built-in minimal scaffold (zero network). 'standard' is a full reference bundle (CSV data source, dataset, IBCS table, chart, style, i18n, assets) in the canonical folder layout — prefer it when a worked example is wanted. May also be owner/repo[/subdir]#ref, a URL, or a local path"`
