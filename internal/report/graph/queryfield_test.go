...
// Regression: the graph package redeclared the query-field union type without
// the null/empty guards spec.QueryField has, so `query: {}` unmarshalled to
// an empty field and read as "absent" instead of "malformed".
