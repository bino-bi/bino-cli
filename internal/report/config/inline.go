...
//   - ChartStructure, ChartTime, ChartScatter, ChartBubble, ChartBullet, Table, Text: spec.dataset can be inline
...
func MaterializeInlineDefinitions(docs []Document) ([]Document, error) {
	...
	for i := range docs {
		doc := &docs[i]
		switch doc.Kind {
		case "ChartStructure", "ChartTime", "ChartScatter", "ChartBubble", "ChartBullet", "Table", "Text":
...
// (ChartStructure, ChartTime, ChartScatter, ChartBubble, ChartBullet, Table, Text).
...
func materializeTreeDocInlines(doc *Document, registry *inlineRegistry) error {
	...
	for nodeIdx, node := range treeDoc.Spec.Nodes {
		...
		// Only process component kinds that can have datasets
		switch node.Kind {
		case "Table", "ChartStructure", "ChartTime", "ChartScatter", "ChartBubble", "ChartBullet":
...
func materializeGridDocInlines(doc *Document, registry *inlineRegistry) error {
	...
	for childIdx, child := range gridDoc.Spec.Children {
		...
		// Only process component kinds that can have datasets
		switch child.Kind {
		case "Table", "ChartStructure", "ChartTime", "ChartScatter", "ChartBubble", "ChartBullet", "Text":
			// Parse the child's spec to check for inline datasets
			var cs childSpec
			if err := json.Unmarshal(child.Spec, &cs); err != nil {
				continue
...
// Tree nodes can contain Table, ChartStructure, ChartTime, ChartScatter, ChartBubble, ChartBullet components with inline datasets.
...
// Grid children can contain Table, ChartStructure, ChartTime, ChartScatter, ChartBubble, ChartBullet, Text components with inline datasets.
