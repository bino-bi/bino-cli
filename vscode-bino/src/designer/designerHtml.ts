...
function getScript(): string {
    ...
    function wireWidgets(form) {
        form.querySelectorAll('.dw[data-widget-kind]').forEach((dw) => {
            ...
            else if (kind === 'filter') { wireFilter(dw); }
            else if (kind === 'groupBy') { wireGroupBy(dw); }
            else if (kind === 'indexColumns') { wireIndexColumns(dw); }
        });
    }
    ...
    // ---- DataSet transform widgets (filter / groupBy / indexColumns) ----

    // Mirror of coerceFilterValue (ibcs.ts): free-text box → the JSON value the
    // schema's dataSetFilterCondition.value expects for the chosen operator.
    // undefined means "omit value" (empty box → IS NULL for equal/notEqual).
    function coerceFilterValue(raw, op) {
        const trimmed = raw.trim();
        if (trimmed === '') { return undefined; }
        if (op === 'in' || op === 'notIn') { return trimmed.split(',').map((s) => s.trim()).filter(Boolean); }
        if (op === 'regex') { return trimmed; }
    ...
    function wireFilter(dw) {
        const rowsEl = q(dw, 'rows');
        if (!rowsEl) { return; }
        const columns = JSON.parse(rowsEl.getAttribute('data-columns') || '[]');
        const groupOpSel = q(dw, 'group-op');

        function objOf(row) {
            if (row.getAttribute('data-row-kind') === 'group') {
                try { return JSON.parse(row.getAttribute('data-group-json') || 'null'); } catch (e) { return null; }
            }
            const o = { column: q(row, 'column').value.trim(), op: q(row, 'op').value };
            const nullChk = q(row, 'value-null');
            if (nullChk && nullChk.checked) { o.value = null; return o; }
            const val = coerceFilterValue(q(row, 'value').value, o.op);
            if (val !== undefined) { o.value = val; }
            return o;
        }
        function emit() {
            const conditions = ownRows(rowsEl).map(objOf).filter((c) => c);
            // The schema requires conditions minItems:1; never emit an empty group
            // (would fire a rejection toast mid-edit). Clear via the JSON tab.
            if (conditions.length === 0) { return; }
            postEdit(dw, { op: (groupOpSel ? groupOpSel.value : 'and'), conditions: conditions });
        }
        if (groupOpSel) { groupOpSel.addEventListener('change', emit); }
        function wireRow(row) {
            const nullChk = q(row, 'value-null');
            if (nullChk) {
                nullChk.addEventListener('change', () => {
                    const valEl = q(row, 'value');
                    if (valEl) { valEl.disabled = nullChk.checked; }
                    emit();
                });
            }
            row.querySelectorAll('input, select').forEach((c) => {
                if (c.closest('[data-dw-role="row"]') !== row) { return; }
                if (c.getAttribute('data-dw-role') === 'value-null') { return; }
                c.addEventListener('change', emit);
            });
            const del = q(row, 'del-row');
            if (del) { del.addEventListener('click', () => { row.remove(); emit(); }); }
        ...
        if (add) {
            add.addEventListener('click', () => {
                const ops = ['equal', 'notEqual', 'gte', 'gt', 'lte', 'lt', 'in', 'notIn', 'regex'];
                const listId = 'dwcols-' + Math.random().toString(36).slice(2, 8);
                const row = document.createElement('div');
                row.className = 'dw-row dw-row-wrap'; row.setAttribute('data-dw-role', 'row'); row.setAttribute('data-row-kind', 'leaf');
                row.innerHTML =
                    '<input class="dw-input dw-sm" type="text" data-dw-role="column" list="' + listId + '" placeholder="column">' +
                    '<datalist id="' + listId + '">' + columns.map((c) => '<option value="' + c + '">').join('') + '</datalist>' +
                    '<select class="dw-select dw-sm" data-dw-role="op">' + ops.map((o) => '<option value="' + o + '">' + o + '</option>').join('') + '</select>' +
                    '<input class="dw-input dw-sm" type="text" data-dw-role="value" placeholder="value">' +
                    '<label class="dw-radio"><input type="checkbox" data-dw-role="value-null"> null</label>' +
                    '<button type="button" class="dw-del" data-dw-role="del-row" title="Remove">\\u00d7</button>';
                rowsEl.appendChild(row);
                wireRow(row); emit();
            });
    ...
    function wireGroupBy(dw) {
        const chipsWrap = q(dw, 'columns');
        const aggRowsEl = q(dw, 'agg-rows');
        const colAdd = q(dw, 'col-add');
        const aggColumns = aggRowsEl ? JSON.parse(aggRowsEl.getAttribute('data-columns') || '[]') : [];

        function columnsArr() {
            const on = chipsWrap ? Array.prototype.slice.call(chipsWrap.querySelectorAll('.dw-chip-on')) : [];
            const list = on.map((c) => c.getAttribute('data-slot'));
            if (colAdd && colAdd.value.trim()) { list.push(colAdd.value.trim()); }
            return list;
        }
        function aggOf(row) {
            const o = { column: q(row, 'agg-column').value.trim(), fn: q(row, 'agg-fn').value, as: q(row, 'agg-as').value.trim() };
            const ob = q(row, 'agg-orderby'); if (ob && ob.value.trim()) { o.orderBy = ob.value.trim(); }
            const od = q(row, 'agg-orderdesc'); if (od && od.checked) { o.orderDesc = true; }
            return o;
        }
        function emit() {
            const cols = columnsArr();
            // schema columns minItems:1 — don't emit an invalid empty group mid-edit.
            if (cols.length === 0) { return; }
            const out = { columns: cols };
            const aggs = aggRowsEl ? ownRows(aggRowsEl).map(aggOf).filter((a) => a.column && a.fn && a.as) : [];
            if (aggs.length > 0) { out.aggregates = aggs; }
            postEdit(dw, out);
        }
        if (chipsWrap) { chipsWrap.querySelectorAll('.dw-chip').forEach((chip) => chip.addEventListener('click', () => { chip.classList.toggle('dw-chip-on'); emit(); })); }
        if (colAdd) {
            colAdd.addEventListener('change', () => {
                const name = colAdd.value.trim();
                if (name) {
                    // Promote free-entry to a chip so the box can take the next column.
                    const chip = document.createElement('button');
                    chip.type = 'button'; chip.className = 'dw-chip dw-chip-on';
                    chip.setAttribute('data-dw-role', 'col-chip'); chip.setAttribute('data-slot', name);
                    chip.textContent = name;
                    chip.addEventListener('click', () => { chip.classList.toggle('dw-chip-on'); emit(); });
        ...
        if (aggRowsEl) { ownRows(aggRowsEl).forEach(wireRow); }
        const add = q(dw, 'add-row');
        if (add && aggRowsEl) {
            add.addEventListener('click', () => {
                const fns = ['sum', 'avg', 'min', 'max', 'first', 'last', 'count', 'countDistinct'];
                const listId = 'dwcols-' + Math.random().toString(36).slice(2, 8);
                const row = document.createElement('div');
                row.className = 'dw-row dw-row-wrap'; row.setAttribute('data-dw-role', 'row');
                row.innerHTML =
                    '<input class="dw-input dw-sm" type="text" data-dw-role="agg-column" list="' + listId + '" placeholder="column or *">' +
                    '<datalist id="' + listId + '">' + aggColumns.map((c) => '<option value="' + c + '">').join('') + '</datalist>' +
                    '<select class="dw-select dw-sm" data-dw-role="agg-fn">' + fns.map((f) => '<option value="' + f + '">' + f + '</option>').join('') + '</select>' +
                    '<input class="dw-input dw-sm" type="text" data-dw-role="agg-as" placeholder="as">' +
                    '<details class="dw-style"><summary>order</summary><div class="dw-style-body">' +
                    '<label class="dw-flabel">orderBy <input class="dw-input dw-sm" type="text" data-dw-role="agg-orderby" placeholder="column"></label>' +
                    '<label class="dw-radio"><input type="checkbox" data-dw-role="agg-orderdesc"> desc</label>' +
                    '</div></details>' +
                    '<button type="button" class="dw-del" data-dw-role="del-row" title="Remove">\\u00d7</button>';
    ...
    function wireIndexColumns(dw) {
        const rowsEl = q(dw, 'rows');
        if (!rowsEl) { return; }
        const columns = JSON.parse(rowsEl.getAttribute('data-columns') || '[]');
        const pairs = JSON.parse(rowsEl.getAttribute('data-index-pairs') || '[]');
        const indexTargets = pairs.map((p) => p.index);

        function objOf(row) {
            const o = { column: q(row, 'column').value.trim() };
            const mode = q(row, 'mode').value;
            if (mode === 'expr') {
                const e = q(row, 'expr').value.trim(); if (e) { o.expr = e; }
                return o;
            }
            o.fn = mode;
            if (mode === 'hash') {
                const of = q(row, 'of').value.trim(); if (of) { o.of = of; }
            } else {
                const ov = q(row, 'over').value.trim(); if (ov) { o.over = ov; }
                const od = q(row, 'over-desc'); if (od && od.checked) { o.overDesc = true; }
                const p = q(row, 'partition').value.trim(); if (p) { o.partition = p.split(',').map((s) => s.trim()).filter(Boolean); }
            }
            return o;
        }
        function emit() { postEdit(dw, ownRows(rowsEl).map(objOf)); }
        function applyMode(row) {
            const mode = q(row, 'mode').value;
            const hash = q(row, 'args-hash'); if (hash) { hash.hidden = mode !== 'hash'; }
            const win = q(row, 'args-window'); if (win) { win.hidden = !(mode === 'rowNumber' || mode === 'rank' || mode === 'denseRank'); }
            const expr = q(row, 'args-expr'); if (expr) { expr.hidden = mode !== 'expr'; }
        }
        function applyPairSuggest(row) {
            const col = q(row, 'column').value.trim();
            const hit = pairs.filter((p) => p.index === col)[0];
            if (!hit) { return; }
            const ofEl = q(row, 'of'); if (ofEl && !ofEl.value) { ofEl.value = hit.source; }
            const ovEl = q(row, 'over'); if (ovEl && !ovEl.value) { ovEl.value = hit.source; }
        }
        function wireRow(row) {
            const mode = q(row, 'mode'); if (mode) { mode.addEventListener('change', () => { applyMode(row); emit(); }); }
            const colEl = q(row, 'column'); if (colEl) { colEl.addEventListener('change', () => { applyPairSuggest(row); emit(); }); }
            row.querySelectorAll('input, select').forEach((c) => {
                if (c.closest('[data-dw-role="row"]') !== row) { return; }
                c.addEventListener('change', emit);
            });
        ...
        if (add) {
            add.addEventListener('click', () => {
                const modes = ['expr', 'hash', 'rowNumber', 'rank', 'denseRank'];
                ...
                const opts = (cols) => cols.map((c) => '<option value="' + c + '">').join('');
                const row = document.createElement('div');
                row.className = 'dw-row dw-row-wrap'; row.setAttribute('data-dw-role', 'row');
                row.innerHTML =
                    '<input class="dw-input dw-sm" type="text" data-dw-role="column" list="' + colListId + '" placeholder="output column">' +
                    '<datalist id="' + colListId + '">' + opts(indexTargets) + '</datalist>' +
                    '<select class="dw-select dw-sm" data-dw-role="mode">' + modes.map((m) => '<option value="' + m + '"' + (m === 'rowNumber' ? ' selected' : '') + '>' + m + '</option>').join('') + '</select>' +
                    '<span data-dw-role="args-hash" hidden>' +
                    '<input class="dw-input dw-sm" type="text" data-dw-role="of" list="' + ofListId + '" placeholder="of (column)">' +
                    '<datalist id="' + ofListId + '">' + opts(columns) + '</datalist></span>' +
                    '<span data-dw-role="args-window">' +
                    '<input class="dw-input dw-sm" type="text" data-dw-role="over" list="' + overListId + '" placeholder="over (column)">' +
                    '<datalist id="' + overListId + '">' + opts(columns) + '</datalist>' +
                    '<label class="dw-radio"><input type="checkbox" data-dw-role="over-desc"> desc</label>' +
                    '<input class="dw-input dw-sm" type="text" data-dw-role="partition" placeholder="partition (comma-separated)"></span>' +
                    '<span data-dw-role="args-expr" hidden>' +
                    '<input class="dw-input dw-sm" type="text" data-dw-role="expr" placeholder="raw SQL expression"></span>' +
                    '<button type="button" class="dw-del" data-dw-role="del-row" title="Remove">\\u00d7</button>';
                rowsEl.appendChild(row);
                wireRow(row);
            });
